...
var subTask = {  
    ...
    updatesubTask: function(id, subTask, callback) {  
        return db.query("update amz_sub_tasks set team_id = ?,task_id = ?,sub_task_name = ?,cf_change = ?,about_cf = ?,op_type = ?,task_status = ?,deletion = ?,last_modify_by = ?,maintain_date=?  where sub_task_id= ?", [subTask.team_id,subTask.task_id,subTask.sub_task_name,subTask.cf_change,subTask.about_cf,subTask.op_type,subTask.task_status,subTask.deletion,subTask.last_modify_by,subTask.maintain_date , id], callback);  
    }  
};  
