...
var Tasks = {  
    ...
    addTask: function(Task, callback) {  
        return db.query("INSERT INTO amz_tasks(team_id,task_name,added_by,status,about_cf,have_st,have_tdi,tdi_type,op_type,deletion, about_chart , create_date,maintain_date , last_modified_by)  VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?)", [Task.team_id, Task.task_name, Task.added_by, Task.status, Task.about_cf, Task.have_st, Task.have_tdi, Task.tdi_type, Task.op_type, Task.deletion, Task.about_chart ,Task.create_date, Task.maintain_date , Task.last_modified_by], callback);  
    },  
    deleteTask: function(id, callback) {  
        return db.query("delete from amz_tasks where amz_tasks.task_id =?", [id], callback);  
    },  
    updateTask: function(id, Task, callback) {  
        return db.query("update amz_tasks set  team_id = ?,task_name = ?,status = ?,about_cf = ?,have_st = ?,have_tdi = ?,tdi_type = ?,op_type = ?,deletion = ?, about_chart = ? , maintain_date = ?, last_modified_by=?  where amz_tasks.task_id = ?", [Task.team_id, Task.task_name, Task.status, Task.about_cf, Task.have_st, Task.have_tdi, Task.tdi_type, Task.op_type, Task.deletion, Task.about_chart ,Task.maintain_date , Task.last_modified_by , id], callback);  
    }  
};  
