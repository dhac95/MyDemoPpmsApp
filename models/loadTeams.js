...
var loadTeams = {  
    getLoadedTeams: function(id, callback) {  
        return db.query('select amz_user_info.s_no, amz_user_info.team_id , amz_teams.team_name from amz_user_info inner join amz_teams on amz_user_info.team_id = amz_teams.team_id where amz_user_info.user_id = ? and amz_user_info.status = 1 and amz_teams.status = 1 and amz_teams.team_deletion = 0 ORDER BY amz_teams.team_name ASC', [id], callback);
    },
