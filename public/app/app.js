...
app.run(['$rootScope', 'toastr', '$http', '$location', 'Idle', 'Notification', function ($rootScope, toastr, $http, $location , Idle, Notification) {
	...
	//	$http.post($rootScope.endPoint + '/api/Menu/GetMenuByRoleId/' + userRole).then(function (response) {

	//		if (response.data.result.length > 0) {
	//			$rootScope.appMenuList = response.data.result;
	//			$.AdminLTE.tree('.sidebar');
	//		} else {
	//			alert('No menu mapped to your login. Contact Admin');
	//			$rootScope.logout();
	//		}
	//	}, function (response) {
	//		alert('Loaded Failed');
...
//Data removal
	...
	$rootScope.checkMenu = function (menuId) {
		for (var i in $rootScope.appMenuList) {
			if ($rootScope.appMenuList[i].Id === menuId) {
				return true;
			}
		}
		return false;
