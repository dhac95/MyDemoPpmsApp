            ...
            if (err) {
                ...
                db.query('SELECT * FROM amz_dc_units where team_id in (?) and task_id in (?) and sub_task_id in (?) and noofdevice in (?) and month = ? and status = 1', [team, task, subTask, deviceCount, tempDate], function (err2, results2, fields2) {
                    if (err2) {
                        ...
                        db.query('INSERT INTO amz_dc_units(month , team_id , task_id , sub_task_id , noofdevice , percentage , status , modified_by , create_date , maintain_date) VALUES (? , ? , ? , ? , ? , ? , ? , ? , ? , ? ) ', [tempDate, team, task, subTask, deviceCount, percentage, 1, actionBy, today, today],
            ...
            if (err5) {
                ...
                db.query('SELECT * FROM amz_dc_units where team_id in (?) and task_id in (?)  and noofdevice in (?) and month = ? and status = 1 ', [team, task, deviceCount, tempDate], function (err6, results6, fields6) {
                    if (err6) {
                        ...
                        db.query('UPDATE amz_dc_units set status = 1 , noofdevice = ? , percentage = ? , maintain_date = ? , modified_by = ?  WHERE team_id in (?) and task_id in (?)  and noofdevice in (?) and month = ?', [deviceCount, percentage, today, actionBy, team, task, deviceCount, tempDate],
                        ...
                        db.query('INSERT INTO amz_dc_units(month , team_id , task_id  , noofdevice , percentage , status , modified_by , create_date , maintain_date) VALUES (? , ? , ? , ? , ? , ? , ? , ? , ? ) ', [tempDate, team, task, deviceCount, percentage, 1, actionBy, today, today],
