                ...
                if (err) {
                    ...
                    db.query('SELECT team_id from amz_teams where team_name = ?', [req.body.team_name], function (e2, r2, f2) {
                        if (e2) {
                            ...
                            db.query('INSERT INTO amz_user_info(user_id , team_id , status , create_date , maintain_date) VALUES (? , ? , ? , ? , ?) ', [1, team, 1, now, now], function (e3, r3, f3) {
                                if (e3) {
                                    res.send({
                                        'code': 403,
                                        'message': e3
                                    ...
                                    db.query('UPDATE amz_login set team_count = team_count + 1 where user_id = ?', [1], function (e4, r4, f4) {
                                        if (e4) {
                                            res.send({
                                                'code': 405,
                                                'message': e4
