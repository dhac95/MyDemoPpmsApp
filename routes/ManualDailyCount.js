...
router.post('/' , function(req , res , next) {
    var team = req.body.team_id , task = req.body.task_id , subTask = req.body.sub_task_id;
...
router.post('/delete', function (req, res, next) {
    db.query('UPDATE amz_daily_target set deletion = 1 where s_no = ?', [req.body.s_no], function (errors, results, fields) {
        if (errors) {
            res.send(errors);
        } else {
            res.send({
