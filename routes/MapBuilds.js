...
router.post('/' , function(req , res , next) {
    ...
    db.query('select build_no from amz_build_mapping where build_no = ?' , [req.body.build_no] , function(e , r , f) {
        if(e) {
            ...
            db.query('INSERT INTO amz_build_mapping(date , team , release_no , build_no , run_status , build_info , create_date , created_by , deletion_status , modified_by) VALUES (? , ? , ? , ? , ? , ? , ? , ? , ? , ?) ' , 
                [myDate, req.body.team, req.body.release_no, req.body.build_no, req.body.run_status, req.body.build_info, req.body.create_date, req.body.created_by, '0' , req.body.modified_by] , function(e2 , r2 , f2){
                    if(e2) {
                        res.send(e2);
                    } else {
                        res.send({
...
router.put('/:id', function (req, res, next) {
    var myDate = moment(req.body.date).format('YYYY-MM-DD');
    db.query('UPDATE amz_build_mapping set date = ? ,  team = ? , release_no = ? , build_no = ? , run_status = ? , build_info = ? , modified_by = ? where s_no = ?' , 
        [myDate, req.body.team, req.body.release_no, req.body.build_no, req.body.run_status, req.body.build_info,  req.body.modified_by , req.params.id] , function (e , r , f) {
            if(e) {
                res.send({
                    "code" : 400 , 
                    "message" : e
